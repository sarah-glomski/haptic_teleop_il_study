#!/usr/bin/env python3
"""
Real-time diffusion policy inference — UMI-MIGRATION pipeline (real charm-lab UMI repo).

Counterpart of inference.py (Aiden/Robomimic pipeline, untouched). Run checkpoints
trained by train_umi.py on data from convert_data_umi.py. Same two-process
architecture and Kinova P-loop control; the differences are the observation/action
representation:

  Aiden pipeline (inference.py)          UMI pipeline (this file)
  ------------------------------------   ------------------------------------------
  obs 'pose' (10,) combined vector       robot0_eef_pos(3) + robot0_eef_rot_axis_angle
    [xyz, rot6d COLUMNS, grip 0..1]        (rot6d ROWS) + robot0_gripper_width (meters)
                                           + robot0_eef_rot_axis_angle_wrt_start (rot6d)
  camera key 'dji_wrist_rgb'             camera key 'camera0_rgb'
  hand-rolled rot6d encode/decode        UMI's own pose_util (pose_to_mat,
                                           mat_to_pose10d, pose10d_to_mat) — rows
                                           convention, tested framework code
  obs frames: last 2 consecutive ~30 Hz  obs frames spaced dt apart (matches training
                                           down_sample_steps spacing)

pose_repr is 'abs' end-to-end (matching config/umi/task/kinova_teleop.yaml), so no
relative-frame composition is needed at decode time; predicted poses are absolute.
...
Gripper: model I/O is physical width in METERS (Robotiq 2F-85):
  ...
  The constant MUST match convert_data_umi.py.

Usage:
    python inference_umi.py --model /path/to/umi_checkpoint.ckpt
    python inference_umi.py --model ... --dt 0.1 --n-action-steps 8
"""
...
# Path setup: REAL UMI repo (charm-lab/HapticTeleopIL) — not Robomimic/dt_ag-main
...
# MUST match convert_data_umi.py (Robotiq 2F-85 stroke).
...
def pose_msg_to_raw(pose_msg: PoseStamped, gripper_norm: float) -> np.ndarray:
    ...
    Matches convert_data_umi.py exactly: scipy from_quat([qx,qy,qz,qw]) -> rotvec
...
def load_policy(model_path: str, num_inference_steps: int = 16):
    ...
    # Sanity: this loader is for UMI-pipeline checkpoints only
    obs_keys = list(cfg.policy.shape_meta["obs"].keys())
    if not any(k.startswith("robot0_") for k in obs_keys):
        raise RuntimeError(
            f"Checkpoint obs keys {obs_keys} are not UMI-schema — this looks like an "
            f"Aiden-pipeline checkpoint. Use inference.py for those.")
    return policy
...
class PolicyNode(Node):
    ...
    def __init__(self, shared_obs: dict, start_time: float, model_path: str, dt: float):
        super().__init__("kinova_policy_node_umi")
