#!/usr/bin/env python3
"""
Launch file for diffusion policy inference — UMI-MIGRATION pipeline.

Counterpart of launch_inference.py (Aiden pipeline, untouched). Starts the same
nodes but runs inference_umi.py, which expects checkpoints trained by
train_umi.py (UMI obs/action schema — robot0_*/camera0_* keys).

Usage:
    python launch_inference_umi.py --model /path/to/umi_checkpoint.ckpt
    python launch_inference_umi.py --latest   # newest *umi* run's latest.ckpt
"""
...
_INFERENCE_SCRIPT = os.path.join(_THIS_DIR, "inference_umi.py")


def find_latest_umi_checkpoint(search_dir: str) -> str:
    """Most recently modified latest.ckpt from a *umi* training run.

    Filters to runs whose path contains 'umi' so an Aiden-pipeline checkpoint
    (incompatible obs schema) can never be picked up by accident —
    inference_umi.py also hard-rejects non-UMI checkpoints at load time.
    """
    ckpts = glob.glob(os.path.join(search_dir, "**", "*.ckpt"), recursive=True)
    umi_ckpts = [c for c in ckpts if "umi" in os.path.basename(os.path.dirname(os.path.dirname(c))).lower()]
    if not umi_ckpts:
        raise FileNotFoundError(
            f"No *umi* run checkpoints found under {search_dir} "
            f"(train one with train_umi.py first, or pass --model explicitly)")
    latest = [c for c in umi_ckpts if os.path.basename(c) == "latest.ckpt"]
    if latest:
        return max(latest, key=os.path.getmtime)
    return max(umi_ckpts, key=os.path.getmtime)
...
def main(argv=sys.argv[1:]):
    ...
    parser.add_argument("--latest",          action="store_true",
                        help="Use the newest *umi* run checkpoint in training/data/outputs/")
    ...
    if args.latest:
        ...
        args.model = find_latest_umi_checkpoint(outputs_dir)
